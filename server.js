...
async function handleRequest(req, res) {
  ...
  if (['GET', 'POST'].includes(req.method) && /^\/api\/.+$/.test(req.url)) {
    if (req.url == '/api/get-posts') {
      ...
      return;
    } else if (req.url == '/api/add-post' && validateSessionID(req, res)) {
  ...
  // handle all other file requests
    ...
    if (/^\/sw\.js(?:[?#].*)?$/.test(req.url)) {
      serveFile('/js/sw.js', 200, { 'cache-control': 'max-age=0' }, req, res).catch(console.error);
      return;
    }
    ...
    if (/^\/(?:login)(?:[#?]|$)/.test(req.url) && validateSessionID(req, res)) {
    ...
    staticServer.serve(req, res, function onStaticComplete(err) {
      if (err) {
        if (req.headers['accept'].includes('text/html')) {
          serveFile('/404.html', 200, { 'X-Not-Found': '1' }, req, res).catch(console.error);
        } else {
          res.writeHead(404);
          res.end();
        }
...
async function addPost(newPostData, req, res) {
  if (newPostData.title.length > 0 && newPostData.post.length > 0) {
    let postTemplate = await fsReadFile(path.join(WEB_DIR, 'posts', 'post.html'), 'utf-8');
    let newPost = postTemplate.replace(/\{\{TITLE\}\}/g, newPostData.title).replace(/\{\{POST\}\}/, newPostData.post);
    ...
    if (postIDs.length > 0) {
      ...
      if (latestYear == year && latestMonth == month && latestDay == day) {
...
function validateSessionID(req, res) {
  if (req.headers.cookie && req.headers.cookie['sessionId']) {
    ...
    if (isLoggedIn == 1 && sessions.includes(sessionID)) {
      req.sessionID = sessionID;

      // update cookie headers
      res.setHeader('Set-Cookie', getCookieHeaders(sessionID, new Date(Date.now() + /*1 hour in ms*/ 3.6e5).toUTCString()));
      return true;
    } else {
      clearSession(req, res);
    }
...
function clearSession(req, res) {
  var sessionID = req.sessionID || (req.headers.cookie && req.headers.cookie.sessionId);
...
async function doLogin(loginData, req, res) {
  ...
  if (loginData.username == 'admin' && loginData.password == 'admin123') {
    ...
    sendJSONResponse({ OK: true }, res, {
      'Set-Cookie': getCookieHeaders(sessionID, new Date(Date.now() + /*1 hour in ms*/ 3.6e5).toUTCString())
    });
  } else {
    sendJSONResponse({ failed: true }, res);
  }
